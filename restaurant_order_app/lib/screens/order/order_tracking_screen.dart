...
class _OrderTrackingScreenState extends State<OrderTrackingScreen> {
  ...
  int _estimatedWaitTime = 20; // minutes
  bool _isOrderReady = false;
  
  // Mock order details - in real app this would come from an API
  ...
  void initState() {
    ...
    _orderDetails = {
      ...
      'receiptCode': '1234', // This would be generated by the backend
    };
    
    // Initial status
    _currentStatus = _statuses[0];
  ...
  // Simulate order progress with a timer
  void _simulateOrderProgress() {
    int statusIndex = 0;
    
    _statusUpdateTimer = Timer.periodic(const Duration(seconds: 5), (timer) {
      // For demo purposes, update status every few seconds
      if (statusIndex < _statuses.length - 1) {
        statusIndex++;
        setState(() {
          _currentStatus = _statuses[statusIndex];
          _estimatedWaitTime = _estimatedWaitTime - 5;
          if (_estimatedWaitTime < 0) _estimatedWaitTime = 0;
  ...
  double _getProgressPercentage() {
    final currentIndex = _statuses.indexOf(_currentStatus);
    return (currentIndex + 1) / _statuses.length;
  }
          ...
          Padding(
            ...
            child: Column(
              children: [
                ...
                // Order status actions based on status
                if (_order.status == OrderStatus.readyForPickup)
                  Padding(
                    ...
                    child: PrimaryButton(
                      text: 'Confirm Pickup',
                      onPressed: () {
                        // Navigate to receipt code verification
                        context.go('/submit-receipt-code/${_order.id}', extra: {
                          'orderId': _order.id,
                          'restaurantName': _order.restaurantName,
                          'totalAmount': _order.total,
                          'receiptCode': _order.receiptCode,
                        });
                      },
                    ),
                  ),
